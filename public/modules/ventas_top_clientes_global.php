...
if ($desde === null || $hasta === null || $desde > $hasta) {
    ...
    echo 'Parámetros: desde, hasta, top=10';
    exit;
}
$pdo = ventas_pdo();
$data = VentasGeneralReportesGraficos::topClientesGlobal($pdo, $desde, $hasta, $top);
$labels = [];
$valores = [];
$pctAcum = [];
...
$pdfName = 'top_clientes_global_' . $desde . '_' . $hasta . '.pdf';
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <title>Top clientes (global)</title>
...
<body>
    <header><h1>Top clientes (todas las zonas)</h1>
        <p class="meta"><?= htmlspecialchars($desde, ENT_QUOTES, 'UTF-8') ?> — <?= htmlspecialchars($hasta, ENT_QUOTES, 'UTF-8') ?>
            · Total: <?= number_format((float) $data['total_valor'], 2, '.', ',') ?></p></header>
    <main>
            ...
            <div class="reporte-tabs">
                ...
                <div class="reporte-tab-panel is-active" data-panel="0">
                    ...
                    <div id="reporte-pdf-root">
                        ...
                        <table>
                            <thead><tr><th>#</th><th>Cod.</th><th>Nombre</th><th>Líneas</th><th>SUM(Valor)</th><th>% total</th><th>% acum.</th></tr></thead>
                            <tbody>
        ...
        function build() {
            ...
            new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [
                        { label: 'SUM(Valor)', data: payload.valores, backgroundColor: 'rgba(59,130,246,0.65)', yAxisID: 'y', order: 2 },
                        { type: 'line', label: '% acumulado', data: payload.pctAcum, borderColor: 'rgba(248,113,113,1)', yAxisID: 'y1', order: 1, tension: 0.15 },
                    ],
                },
                options: {
