...
$pdo = ventas_pdo();
$data = VentasGeneralReportesGraficos::topCorporativo($pdo, $desde, $hasta, $top);
$labels = [];
$valores = [];
foreach ($data['filas'] as $f) {
    $lab = (string) ($f['nombre_coorporativo'] ?? '');
...
$pdfName = 'ventas_corporativo_' . $desde . '_' . $hasta . '.pdf';
?>
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="utf-8">
    <title>Ventas por corporativo</title>
...
<body>
    <header><h1>Ventas por NombreCoorporativo</h1>
            ...
            <div class="reporte-tabs">
                ...
                <div class="reporte-tab-panel is-active" data-panel="0">
                    ...
                    <div id="reporte-pdf-root">
                        <h2 class="pdf-h2">Top corporativo</h2>
                        <table><thead><tr><th>#</th><th>Cod.</th><th>Nombre corporativo</th><th>Líneas</th><th>SUM(Valor)</th></tr></thead><tbody>
                            <?php $i = 0; foreach ($data['filas'] as $f) { $i++; ?>
                            <tr>
                                <td><?= $i ?></td>
                                <td><?= htmlspecialchars((string) ($f['cod_coorporativo'] ?? ''), ENT_QUOTES, 'UTF-8') ?></td>
                                <td><?= htmlspecialchars((string) ($f['nombre_coorporativo'] ?? ''), ENT_QUOTES, 'UTF-8') ?></td>
    ...
    (function () {
        ...
        function build() {
            ...
            new Chart(ctx, {
                type: 'bar',
                data: { labels: payload.labels, datasets: [{ label: 'SUM(Valor)', data: payload.valores, backgroundColor: 'rgba(234,179,8,0.8)' }] },
